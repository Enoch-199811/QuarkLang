...
func TestRecursionLimitError(t *testing.T) {
	_, err := runSrc(t, "fn f(n int) int { return f(n + 1); }\nfn main(io IOStream) { io.println(f(0)); }")
...
func TestChannelCapLimit(t *testing.T) {
	_, err := runSrc(t, "fn main(io IOStream) { c Channel = taskm.channel(2000000000); }")
...
func TestNewCapLimit(t *testing.T) {
	_, err := runSrc(t, "fn main(io IOStream) { p pointer List<int> = new int[66000000]; }")
