...
- ✅ 2026-08-30 compiler v0.2：跨系统编译器后端定为 **LLVM**（不经 C 转译）——`qkc` 生成文本 LLVM IR（零依赖手写发射器），`llc`/`clang` 产出原生二进制，`-run` 一键编译执行；支持 main + io.println 子集与算术，含 lli 全链路测试。
- ✅ 2026-08-30 compiler 扩展：**变量（int/String 声明与赋值，alloca/store/load）与控制流（if/else、while，基本块 + br）**、比较（icmp）、`&&/||/!`、布尔字面量（select true/false 输出）、格式串按编译后实际类型生成（7 项测试全绿，llvm-as 语法校验 + lli/clang 全链路）。
